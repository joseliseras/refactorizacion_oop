...
class Mapa:
    ...
    def agregar_obstaculo_usuario(self):
        while True:
            try:
                ...
                cordenada_x = input("Ingrese la coordenada x, o presione la tecla 's' para salir: ")
                if cordenada_x.lower() == "s":
                    return
                cordenada_x = int(cordenada_x)

                cordenada_y = input("Ingrese la coordenada y, o presione la tecla 's' para salir: ")
    ...
    def eliminar_obstaculo(self, cordenada_x, cordenada_y):
        if 0 <= cordenada_x < self.size and 0 <= cordenada_y < self.size:
            if self.mapa[cordenada_x][cordenada_y] != 0:
                tipo_anterior = self.mapa[cordenada_x][cordenada_y]
                self.mapa[cordenada_x][cordenada_y] = 0
                tipo_nombre = self.tipos_nombres.get(tipo_anterior, "desconocido")
                print(f"Ha eliminado el obstáculo {tipo_nombre} en las coordenadas x:{cordenada_x} y:{cordenada_y}")
                return True
            else:
                print('No hay obstáculo para eliminar en esa ubicación')
                return False
        else:
            print('No se pudo eliminar obstáculo, porque está fuera del mapa')
            return False

    def eliminar_obstaculo_usuario(self):
        obstaculos_eliminados = 0
        while True:
            try:
                print("ELIMINE OBSTACULOS - Al finalizar presione la tecla 's' para salir.")
                cordenada_x = input("Ingrese la coordenada x del obstáculo a eliminar, o presione la tecla 's' para salir: ")
                if cordenada_x.lower() == "s":
                    break
                cordenada_x = int(cordenada_x)

                cordenada_y = input("Ingrese la coordenada y del obstáculo a eliminar, o presione la tecla 's' para salir: ")
                if cordenada_y.lower() == "s":
                    break
                cordenada_y = int(cordenada_y)

                if self.eliminar_obstaculo(cordenada_x, cordenada_y):
        ...
        if obstaculos_eliminados > 0:
            print("SU MAPA DESPUÉS DE ELIMINAR OBSTÁCULOS ES:")
            self.imprimir_mapa()
        else:
            print("No se eliminaron obstáculos.")
...
class CalculadoraRutas:
    ...
    def costo_movimiento(self, hasta):
        tipo_celda = self.mapa.mapa[hasta[0]][hasta[1]]
...
if __name__ == "__main__":
    ...
    # Eliminar obstáculos desde input hasta que el usuario decida salir
    mapa.eliminar_obstaculo_usuario()
    ...
    ruta_mas_corta = calculadora.buscar_ruta_mas_corta(inicio, objetivo)
